...
router.all("/rider-earnings", async (req: Request, res: Response) => {
  try {
    const targetUrl = `${CLIENT_STORE_SERVICE_URL}/api${req.path}`;
...
// Proxy client rider mapping requests to client-store service
router.all("/client-rider-mappings", async (req: Request, res: Response) => {
  try {
    const targetUrl = `${CLIENT_STORE_SERVICE_URL}/api${req.path}`;
...
// Note: Rider earnings functionality in client-store service
