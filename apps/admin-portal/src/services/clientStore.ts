...
class ClientStoreService {
  ...
  async getClientStats(params?: { clientType?: string }): Promise<ApiResponse<ClientStats>> {
  ...
  async createStore(store: Partial<Store>): Promise<ApiResponse<Store>> {
    const response = await this.api.post('/api/stores', store)
    return response.data
  }

  async updateStore(id: string, store: Partial<Store>): Promise<ApiResponse<Store>> {
    const response = await this.api.put(`/api/stores/${id}`, store)
    return response.data
  }
  ...
  async getStoreStats(params?: { clientId?: string }): Promise<ApiResponse<StoreStats>> {
  ...
  async getRiderEarningsByRider(riderId: string, params?: {
    dateFrom?: string
    dateTo?: string
    paymentStatus?: string
  }): Promise<ApiResponse<{ earnings: RiderEarning[], summary: any }>> {
    const response = await this.api.get(`/api/rider-earnings/rider/${riderId}`, { params })
    return response.data
  }

  async getRiderEarningsByStore(storeId: string, params?: {
    dateFrom?: string
    dateTo?: string
  }): Promise<ApiResponse<RiderEarning[]>> {
    const response = await this.api.get(`/api/rider-earnings/store/${storeId}`, { params })
    return response.data
  }

  async createRiderEarning(earning: Partial<RiderEarning>): Promise<ApiResponse<RiderEarning>> {
    const response = await this.api.post('/api/rider-earnings', earning)
    return response.data
  }

  async updateRiderEarning(id: string, earning: Partial<RiderEarning>): Promise<ApiResponse<RiderEarning>> {
  ...
  async getWeeklyRiderSummary(riderId: string, params?: {
    year?: number
    week?: number
  }): Promise<ApiResponse<any>> {
    const response = await this.api.get(`/api/rider-earnings/weekly/${riderId}`, { params })
    return response.data
  }
  ...
  async generateWeeklyReport(data: {
    ...
    const response = await this.api.post('/api/rider-earnings/reports/weekly', data)
    return response.data
  }
}
