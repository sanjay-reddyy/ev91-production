...
router.get(
  "/stock-levels/:storeId/:sparePartId",
  authMiddleware,
  asyncHandler(inventoryController.getStockLevel.bind(inventoryController))
);

// GET /api/v1/inventory/stock-levels/part/:sparePartId - Get all stock levels for a part (basic auth only)
router.get(
  "/stock-levels/part/:sparePartId",
  authMiddleware,
  asyncHandler(
    inventoryController.getStockLevelsByPart.bind(inventoryController)
  )
);
...
router.post(
  ...
  authMiddleware,
  asyncHandler(
    inventoryController.releaseReservedStock.bind(inventoryController)
  )
);
