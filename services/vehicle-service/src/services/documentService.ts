...
console.log(
  `- Access Key ID: ${process.env.AWS_ACCESS_KEY_ID ? process.env.AWS_ACCESS_KEY_ID.substring(0, 5) + "..." : "Not set"}`
);
...
export class DocumentService {
  ...
  static async uploadVehicleDocument(params: UploadDocumentParams) {
    ...
    try {
      ...
      // Upload to S3
      console.log(
        `DEBUG: Attempting to upload to bucket: ${bucketName} in region: ${process.env.AWS_REGION}`
      );
      const uploadParams = {
        Bucket: bucketName,
        Key: fileName,
