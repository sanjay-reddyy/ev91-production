...
export const createRiderEarning = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const {
    riderId,
    storeId,
    orderId,
  ...
  if (!riderId || !storeId || !orderId) {
    throw createError('RiderId, storeId, and orderId are required', 400, 'MISSING_REQUIRED_FIELDS');
  }
  ...
  const existingEarning = await prisma.riderEarning.findUnique({
    where: { orderId }
  });

  if (existingEarning) {
    throw createError('Earning record already exists for this order', 409, 'EARNING_EXISTS');
  }
  ...
  const riderEarning = await prisma.riderEarning.create({
    data: {
      ...
      deliveryStartTime: deliveryStartTime ? new Date(deliveryStartTime) : null,
...
export const getRiderEarnings = asyncHandler(async (req: Request, res: Response) => {
  const {
    page = '1',
    limit = '10',
    riderId,
...
export const getRiderEarningById = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const updateRiderEarning = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  ...
  if (updateData.baseEarning) updateData.baseEarning = parseFloat(updateData.baseEarning);
  if (updateData.distanceBonus) updateData.distanceBonus = parseFloat(updateData.distanceBonus);
  if (updateData.timeBonus) updateData.timeBonus = parseFloat(updateData.timeBonus);
  if (updateData.storeOfferBonus) updateData.storeOfferBonus = parseFloat(updateData.storeOfferBonus);
  if (updateData.evBonus) updateData.evBonus = parseFloat(updateData.evBonus);
  if (updateData.peakTimeBonus) updateData.peakTimeBonus = parseFloat(updateData.peakTimeBonus);
  if (updateData.qualityBonus) updateData.qualityBonus = parseFloat(updateData.qualityBonus);
  if (updateData.penaltyAmount) updateData.penaltyAmount = parseFloat(updateData.penaltyAmount);
  if (updateData.bonusEarning) updateData.bonusEarning = parseFloat(updateData.bonusEarning);
  if (updateData.totalEarning) updateData.totalEarning = parseFloat(updateData.totalEarning);
  if (updateData.distanceTraveled) updateData.distanceTraveled = parseFloat(updateData.distanceTraveled);
  if (updateData.fuelUsed) updateData.fuelUsed = parseFloat(updateData.fuelUsed);
  if (updateData.energyUsed) updateData.energyUsed = parseFloat(updateData.energyUsed);

  // Handle date fields
  if (updateData.orderDate) updateData.orderDate = new Date(updateData.orderDate);
  if (updateData.deliveryStartTime) updateData.deliveryStartTime = new Date(updateData.deliveryStartTime);
  if (updateData.deliveryEndTime) updateData.deliveryEndTime = new Date(updateData.deliveryEndTime);
...
export const deleteRiderEarning = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
...
export const getRiderEarningsByRider = asyncHandler(async (req: Request, res: Response) => {
...
export const getRiderEarningsByStore = asyncHandler(async (req: Request, res: Response) => {
...
export const getWeeklyRiderSummary = asyncHandler(async (req: Request, res: Response) => {
  const { riderId } = req.params;
  ...
  if (!weeklySummary) {
    ...
    const summaryData = earnings.reduce((acc, earning) => {
      acc.totalEarnings += earning.totalEarning;
      acc.totalOrders += 1;
      acc.totalBaseEarnings += earning.baseEarning;
      acc.totalBonuses += earning.bonusEarning;
    ...
    weeklySummary = await prisma.weeklyRiderSummary.create({
      data: {
        ...
        weekEndDate: endDate,
        ...summaryData,
        averageEarningPerOrder: summaryData.totalOrders > 0 ? summaryData.totalEarnings / summaryData.totalOrders : 0,
        metadata: {}
      }
    });
  }
...
export const generateWeeklyReport = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const reports = await Promise.all(
    riderIds.map(async (riderId: string) => {
      ...
      return {
        ...
        summary: {
          ...summary,
          averageEarningPerOrder: summary.totalOrders > 0 ? summary.totalEarnings / summary.totalOrders : 0
        }
      };
    })
  );
