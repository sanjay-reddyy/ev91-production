...
export class KycService {
  ...
  async uploadDocument(
    ...
    try {
      ...
      console.log(`🔄 Processing document upload for rider ${riderId} (type: ${documentType})`);
  ...
  /**
   * Check KYC status for rider (legacy method)
   */
  async checkKycStatus(riderId: string) {
    const rider = await prisma.rider.findUnique({ where: { id: riderId } });
  ...
  /**
   * Get KYC status and details for a rider
   * @param riderId Rider ID to check KYC status for
   * @returns KYC status and document details
   */
  async getKycStatus(riderId: string) {
    ...
    for (const doc of kycDocuments) {
      if (!documentsByType[doc.documentType] || 
          new Date(doc.updatedAt) > new Date(documentsByType[doc.documentType].updatedAt)) {
    ...
    return {
      ...
      completionPercentage: Math.round(
        (Object.keys(documentsByType).length / requiredDocumentTypes.length) * 100
      )
    };
  }
}
