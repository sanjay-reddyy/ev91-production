...
export class CitySyncService {
  ...
  static async getSyncStatus() {
    try {
      ...
      // Get rider statistics by city
      const ridersByCity = await prisma.rider.groupBy({
        by: ["city"],
        _count: {
          id: true,
        },
      ...
      return {
        statistics: {
          ...
          ridersDistribution: ridersByCity,
        },
  ...
  static async getAllCities() {
    try {
      ...
      console.log(`📋 Retrieved ${cities.length} cities from rider service`);
      return cities;
    } catch (error) {
